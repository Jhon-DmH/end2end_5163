...
class HashController:
    ...
    def verify_file_hash(file_path, expected_hash, algorithm='sha256'):
        ...
        try:
            ...
            actual_hash = HashController.calculate_file_hash(original_content, algorithm)
        ...
        for root, _, files in os.walk(directory):
            for file in files:
                ...
                try:
                    with open(file_path, 'rb') as file:
                        original_content = file.read()
                    file_hash = HashController.calculate_file_hash(original_content, algorithm)
                    # 使用相对路径作为键
    ...
    def verify_directory(directory, hash_file=None, algorithm='sha256'):
        ...
        for file_path, expected_hash in stored_hashes.items():
            if os.path.exists(file_path):
                is_valid = HashController.verify_file_hash(file_path, expected_hash, stored_algorithm)
...
if __name__ == "__main__":
    ...
    try:
        ...
        file_hash = HashController.calculate_file_hash(file_path)
        print(f"文件 {file_path} 的SHA256哈希值: {file_hash}")
        
        # 验证哈希值
        is_valid = HashController.verify_file_hash(file_path, file_hash)
        print(f"哈希值验证结果: {'通过' if is_valid else '失败'}")
        
        # 为目录生成哈希文件
        directory = "."
        HashController.generate_hash_file(directory)
        print(f"已为目录 {directory} 生成哈希文件")
        
        # 验证目录
        all_passed, results = HashController.verify_directory(directory)
